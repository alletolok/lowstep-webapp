...
function getInitial(user: TgUser | null) {
  const base = (user?.first_name?.trim() || user?.username?.trim() || "A").trim();
  return base.charAt(0).toUpperCase();
}

export default function TopBar({ user, onSupportClick, onReviewsClick }: Props) {
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbarCenter">
        <div className="topbarUserPill" role="button" tabIndex={0} aria-label="Profile">
          <div className="topbarAvatar" aria-hidden="true">
            {avatarUrl ? (
              <img className="topbarAvatarImg" src={avatarUrl} alt="" />
            ) : (
              <span className="topbarAvatarFallback">{getInitial(user)}</span>
            )}
          </div>
